...
class Camera:
    ...
    def __init__(
        ...
        Args:
            mode (str): Camera mode (fps, orbital, free, fixed)
            position (Vector3D): Initial camera position
            target (Vector3D): Initial camera target
            up (Vector3D): Camera up vector
            fovy (float): Field of view Y in degrees
        """
        self.mode = mode
