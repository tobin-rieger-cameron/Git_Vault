...
write to stderr — that corrupts the TUI's rendering. A file handler on the "program_files" logger
...
def configure(log_path: Path, level: int = logging.DEBUG) -> None:
    """Attach a file handler to the "program_files" logger; child loggers propagate into it by default."""
    logger = logging.getLogger("program_files")
