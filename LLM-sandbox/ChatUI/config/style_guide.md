...
- **A comment or docstring must stand on its own.** If understanding it requires jumping to another module or a design doc, it isn't doing its job — put that context in `config/` docs, not scattered inline. In particular, **never cite an author, book, methodology, or design/research doc as provenance for a choice** (`per Norman`, `(CLIG)`, `matching the artifact`, `see ui_style_guide.md for the research`) — state the actual reasoning inline instead; the citation ages into noise the moment the source moves or the reader doesn't have it. The one exception is a short trailing pointer to an authoritative *in-repo spec*, and only when it points to concrete reference material a maintainer would need (`per CLAUDE.md's retrieval table`), never merely to justify the decision.
...
- Every **public** module, function, class, and method gets a docstring — with two deliberate carve-outs, each governed by the same anti-noise principle as the comments section (a docstring that only restates the name is the same failure as a comment that restates the code). These are not a licence to skip real docstrings:
  - A **trivial marker class** — an exception subclass that only adds a name, or a `@dataclass` DTO whose fields are fully self-describing — is exempt when the name plus the module docstring already make it obvious. A docstring earns its place on such a class only when it adds what the fields can't: a unit, an invariant, or the set of allowed values (`Chunk`'s "score is Chroma's similarity score, not a raw distance"; `Override`'s "kind is e.g. tags/placement/wikilink"). Apply the same test to a `@dataclass`: docstring it only where a field's meaning isn't carried by its name and type.
  - A **non-public (`_`-prefixed) function** doesn't need one, and by default doesn't get a comment either — only where the *why*, or a non-obvious return contract (a multi-value tuple, a sentinel value), isn't already carried by the name. When it does get a docstring, it's still a single imperative line, and any design *why* moves out to a `#` comment rather than padding the docstring.
...
- **Module docstrings: one line, noun phrase, no provenance.** A module docstring names what the file *is*, in one line — never a multi-paragraph history of how it came to be or which design doc informed it (that history lives in `config/changelog.md`). Skip it entirely for a module whose only honest docstring would restate the filename (a plain unit-test module); write one where the module carries a non-obvious contract worth stating (an integration-test module that hits real services and self-skips when they're unreachable).
