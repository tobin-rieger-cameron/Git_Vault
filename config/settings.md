...
FIX: _cmd_harvest topic extraction is broken — the INDEX topics field contains raw question text like "What is the difference between a taxonomy; What is fine-tuning in the context of large language models" rather than clean concept keywords. Fix _cmd_harvest so that after splitting each topic phrase on ";", it strips leading interrogative words (strip prefixes: "what is", "what are", "how does", "how do", "how is", "why is", "what", "how", "why") case-insensitively, then extracts only the meaningful noun portion (first 40 chars of what remains). Also fix the vault-file matching: instead of only exact slug lookup, check whether any single word from the cleaned topic (length >= 5) appears as a substring in any existing vault stem, and use that file if found.
FIX: _finalize_session topic extraction is broken — it currently stores raw question text truncated at 80 chars. Replace the topics line `topics = "; ".join(q[:80].replace("\n", " ") for q in questions[:3])` with logic that strips interrogative starters (remove leading "what is", "what are", "how does", "how do", "how is", "why is", "what", "how", "why" case-insensitively), then takes the first 50 chars of the remaining noun phrase, so the topics field contains concept summaries rather than full questions.
